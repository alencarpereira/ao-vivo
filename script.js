...
function analisarJogo(dados) {
    ...
    // OVER (ajustado pelo tempo restante)
    let over15 = "Baixo";
    let over25 = "Baixo";
    ...
    if (indicePressao > 2 && Math.abs(momentum) > 0.8) sinal = "PRESSÃO FORTE";
...
function atualizarSinais(r) {
    ...
    // Próximo Gol
    document.getElementById("proximoGol").querySelector("p").innerText = "Próximo Gol: " + r.proximoGol;

    // Alerta Gol
    document.getElementById("alertaGol").querySelector("p").innerText = "Alerta Gol: " + r.alertaGol;
}